...
- **国产模型额度授权**：左侧厂商导航列出阿里云百炼、月之暗面、小米 MiMo、智谱、火山方舟、
  ...
  Kimi Coding Plan 会员权益、Weekly/5H 用量及各自重置时间；百炼 Token Plan 显示总额度和固定重置时间，Coding Plan 仅在控制台实际返回窗口时显示 5H/WK。它们在启动时立即读取、随后每 2 分钟后台刷新；自动或手动请求至少间隔 60 秒，供应商限流时退避 5 分钟。小米已预留响应规则但尚未用真实订阅账号验证。其余厂商提供
