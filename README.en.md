...
| | |
|---|---|
| <img src="docs/images/feature1.jpg" width="360" alt="AI status"> | **AI status & quota**<br>Pet is walking = the AI is working. Claude/Codex show each provider-reported 5H/WK usage window with its reset countdown; Codex also shows the remaining reset-credit count returned by the official endpoint. The domestic-model submenu selects exactly one vendor. Alibaba Bailian Token Plan shows total usage and its fixed reset time; a Coding Plan shows 5H/WK only when those windows are actually returned. Moonshot shows the Kimi membership tier plus Coding Plan Weekly/5H usage and reset times. Authenticated values refresh every 2 minutes while the last successful result remains cached; unknown values are never estimated. |
