...
| | |
|---|---|
| <img src="docs/images/feature1.jpg" width="360" alt="AI 工作状态"> | **AI 工作状态与额度**<br>桌宠动起来 = AI 正在干活。Claude/Codex 显示供应商返回的真实额度；国产模型页可在厂商子菜单中单选。阿里云百炼显示千问 Token Plan，月之暗面显示 Kimi Coding Plan 的 Weekly/5h；授权后读取并缓存供应商页面返回的准确百分比。未知额度显示 `--`，不做估算。 |
...
| | **自动屏保与 Codex 提醒**<br>Windows 可按真实键鼠空闲时间在 1/5/10/30/60 分钟后进入移动时钟屏保；键鼠恢复后回到原页面。立即预览不会被正在工作的桌宠覆盖。Codex 等待审批时全局切入红色边框提醒；一个或多个任务完成时 Windows 播放系统提示音，屏幕持续显示平滑绿色脉冲和桌宠庆祝动作，切回 Codex 后恢复原页面。 |
